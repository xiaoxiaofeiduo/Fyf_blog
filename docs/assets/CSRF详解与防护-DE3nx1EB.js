...
CSRF 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
CSRF 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 对所有状态变更请求启用 CSRF Token。
- Cookie 设置 SameSite=Lax 或 Strict，并配合 Secure 和 HttpOnly。
- 关键操作增加二次确认、验证码或重新认证。
- 严格校验 Origin 和 Referer，作为辅助防线。
- 避免使用 GET 执行状态变更操作。
