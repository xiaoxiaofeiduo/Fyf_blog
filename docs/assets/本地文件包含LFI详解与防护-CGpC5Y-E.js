...
LFI 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
LFI 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 不要用用户输入决定 include、require、模板路径。
- 使用固定模板枚举或资源 ID。
- 路径归一化后校验允许目录和允许扩展名。
- 关闭远程包含和危险解释配置。
- 避免把上传目录、日志目录纳入可包含路径。
