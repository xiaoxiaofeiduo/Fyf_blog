...
目录遍历 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
目录遍历 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 不要让用户直接控制文件系统路径。
- 使用文件 ID 映射实际路径。
- 路径归一化后校验是否仍位于允许目录。
- 拒绝 ../、绝对路径、URL 编码绕过、双重编码绕过和符号链接绕过。
- 文件读取服务运行在低权限账号下，并限制可访问目录。
