...
tags: 漏洞库, Web安全, Email注入, 安全防护, 安全测试
cover: app-orange
readingTime: 13 分钟
featured: false
---
...
Email 注入 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
Email 注入 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 禁止用户输入直接进入邮件头字段。
- 过滤 CR/LF 控制字符，防止新增邮件头。
- 收件人、主题、正文分别使用不同校验策略。
- 使用成熟邮件库构造邮件，而不是手工拼接 MIME 内容。
- 对联系人表单、反馈表单、邀请邮件做滥用频率限制。
