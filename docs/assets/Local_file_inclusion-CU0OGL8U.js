...
注意：不要依赖黑名单、编码或输入验证/清理方法（例如过滤）来防止包含本地文件。 例如，不要试图限制或强制执行文件扩展名或阻止特殊字符序列作为您免受 LFI 的唯一保护。 攻击者可以使用多种不同的方法（例如 URL 编码和包装器（例如 php://filter）来绕过此类过滤器。`;export{n as default};
