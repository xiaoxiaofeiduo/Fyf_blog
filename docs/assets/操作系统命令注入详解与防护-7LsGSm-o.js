...
操作系统命令注入 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
操作系统命令注入 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 优先使用语言内置 API 替代 shell 命令。
- 必须调用命令时使用参数数组，不通过 shell 拼接字符串。
- 对参数使用 allowlist，限制字符集、长度和枚举值。
- 运行进程使用低权限账号和隔离环境。
- 记录命令调用审计日志并限制执行超时。
