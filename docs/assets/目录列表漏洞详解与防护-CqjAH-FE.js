...
目录列表漏洞 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
目录列表漏洞 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 关闭 Web 服务器目录索引功能。
- 为静态资源目录配置最小暴露范围。
- 禁止把源码、备份、日志、配置文件放在 Web 根目录。
- 为敏感目录配置访问控制和认证。
- 上线前扫描目录索引、备份文件和敏感扩展名。
