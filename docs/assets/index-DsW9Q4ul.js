...
注意：不要依赖黑名单、编码或输入验证/清理方法（例如过滤）来防止包含本地文件。 例如，不要试图限制或强制执行文件扩展名或阻止特殊字符序列作为您免受 LFI 的唯一保护。 攻击者可以使用多种不同的方法（例如 URL 编码和包装器（例如 php://filter）来绕过此类过滤器。`,e_=`---
title: NoSQL 注入
date: 2026-05-10
...
您可以通过基于白名单（而非黑名单）使用适当的输入/输出数据验证和清理来最大程度地降低攻击风险并减轻其后果。虽然在这种情况下您无法完全避免 RCE 漏洞。`,r_=`---
title: 远程文件包含
date: 2026-05-07
...
此外，您应该将您的环境配置为不显示数据库错误——这有助于缓解甚至未发现的基于错误的 SQL 注入漏洞。`,s_=`---
title: 服务器端请求伪造 (SSRF)
date: 2026-05-02
...
架构控制：如果您的应用程序仅使用 HTTP 或 HTTPS 发出请求，则仅允许这些 URL 架构。 如果禁用所有其他 URL 模式，攻击者将无法使用 Web 应用程序使用具有潜在危险的模式（例如文件、字典、ftp 和 gopher）发出请求。`,l_=`---
title: 盲跨站脚本
date: 2026-04-30
...
请注意，并非所有 DOM 元素都有安全的输出方法。 在某些情况下，您必须避免使用不受信任的数据。 例如，您绝不能将任何不受信任的数据写入 eval 或 execScript 等接收器。`,d_=`---
title: 反射/非持久性跨站点脚本
date: 2026-04-28
...
- 他们可以创建一个有效负载，将用户发送到模仿您应用程序登录页面的恶意页面。 然后，他们可以将此恶意 URL 发送给您的内部用户，甚至是您的 CEO。 即使您的一个用户上当受骗，攻击者也将获得他们的凭据以升级攻击。 最终，这可能会让恶意行为者获得对您组织中其他计算机系统的访问权限。`,f_=`---
title: 存储的/持久的跨站点脚本
date: 2026-04-27
...
请注意，如果您的 Web 应用程序为内部用户（例如公司员工）提供服务，则成功的攻击可能会让恶意行为者升级并可能获得对您组织中其他计算机系统的访问权限。`,p_=`---
title: XSS
date: 2026-05-01
