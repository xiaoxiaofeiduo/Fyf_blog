...
tags: 漏洞库, Web安全, NoSQL注入, 安全防护, 安全测试
cover: app-yellow
readingTime: 14 分钟
featured: false
---
...
NoSQL 注入 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
NoSQL 注入 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 严格校验输入类型，拒绝对象替代字符串的查询参数。
- 使用安全查询构造器，避免把用户输入直接合并进查询对象。
- 禁止用户控制操作符，例如 $ne、$gt、$where。
- 关闭服务端 JavaScript 查询能力或限制其使用。
- 对认证、搜索、筛选接口做操作符注入测试。
