...
tags: 漏洞库, Web安全, HTML注入, 安全防护, 安全测试
cover: app-pink
readingTime: 14 分钟
featured: false
---
...
HTML 注入 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
HTML 注入 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 默认对 HTML 上下文输出进行转义。
- 富文本场景使用白名单 HTML Sanitizer。
- 禁止用户控制 base、form、iframe、object 等高风险标签。
- 对 URL 属性限制协议，只允许 http、https 等业务需要协议。
- 启用 CSP 降低注入后进一步扩大影响的风险。
