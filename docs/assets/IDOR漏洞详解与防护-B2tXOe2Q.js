...
tags: 漏洞库, Web安全, IDOR, 访问控制, 安全防护, 安全测试
cover: app-teal
readingTime: 14 分钟
featured: false
---
...
IDOR 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
IDOR 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 每次访问对象前都进行服务端授权校验。
- 不要依赖前端隐藏按钮、不可猜 ID 或 UUID 作为权限控制。
- 对象查询必须绑定当前用户、组织、租户或角色范围。
- 批量接口、导出接口、下载接口也必须做逐对象授权。
- 为越权访问建立自动化测试矩阵。
