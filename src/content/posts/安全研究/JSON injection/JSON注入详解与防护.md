...
JSON 注入 是 Web 安全测试和安全防护中常见的一类风险。它通常出现在应用处理用户输入、服务端资源、浏览器行为、数据库查询或系统边界时。本文将原有漏洞条目内容整合为一篇独立文章，覆盖漏洞原理、攻击方式、潜在影响、测试关注点和防护实践。
...
JSON 注入 的防护不应只依赖单点过滤，而应从输入校验、上下文处理、权限边界、运行环境和监控审计多个层面收敛风险。

- 使用 JSON 序列化器生成响应，不手工拼接 JSON 字符串。
- 对字段类型、长度、枚举值进行服务端校验。
- 避免把用户输入作为 JSON 结构片段解析执行。
- 设置正确 Content-Type，避免 JSON 被当作脚本执行。
- 对客户端解析逻辑做异常输入测试。
